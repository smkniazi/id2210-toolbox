...
import se.sics.p2ptoolbox.simulator.dsl.events.KompicsSimulatorEvent;
import se.sics.p2ptoolbox.simulator.dsl.events.PeriodicSimulatorEvent;
...
public final class P2pSimulator extends ComponentDefinition implements Simulator {
    ...
    private void executeStochasticProcessTerminatedEvent(
        ...
        SimulationTerminatedEvent terminationEvent = event.getTerminationEvent();
    ...
    private void executeKompicsEvent(KompicsEvent kompicsEvent) {
        ...
        if (Timeout.class.isAssignableFrom(kompicsEvent.getClass())) {
            ...
            logger.debug("Triggered Timeout: {} {}", kompicsEvent, timeout.getTimeoutId());
    ...
    private void executePeriodicSimulatorEvent(PeriodicSimulatorEvent periodic) {
        ...
        if (Timeout.class.isAssignableFrom(periodic.getEvent().getClass())) {
            ...
            try {
                ...
                logger.error("Timeout is not clonable - kompics internal error");
                System.exit(1);
            }
    ...
    Handler<ScheduleTimeout> handleST = new Handler<ScheduleTimeout>() {
        ...
        public void handle(ScheduleTimeout event) {
            ...
            KompicsSimulatorEvent timeout = new KompicsSimulatorEvent(event.getTimeoutEvent(), CLOCK + event.getDelay());
    ...
    Handler<CancelTimeout> handleCT = new Handler<CancelTimeout>() {
        ...
        public void handle(CancelTimeout event) {
            ...
            logger.debug("CancelTimeout: {}. AT={}", timeoutId, activeTimers.keySet());
    ...
    Handler<CancelPeriodicTimeout> handleCPT = new Handler<CancelPeriodicTimeout>() {
        ...
        public void handle(CancelPeriodicTimeout event) {
            ...
            if (!removed) {
                logger.warn("Cannot find periodic timeout {}", event.getTimeoutId());
            }
        }
    };
