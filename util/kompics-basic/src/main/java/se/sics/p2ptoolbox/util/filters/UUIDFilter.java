 ...
 */
public class UUIDFilter extends ChannelFilter<Msg, UUID> {
    ...
    @Override
    public UUID getValue(Msg message) {
        Address dst = message.getHeader().getDestination();
        if(dst instanceof UUIDIdentifiable) {
            return ((UUIDIdentifiable)dst).getId();
        } else {
